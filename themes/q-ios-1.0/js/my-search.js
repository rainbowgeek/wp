define( [ 'jquery', 'core/theme-app', 'root/config' ], function( $, App, Config ) {
	...
	var current_search = {
		...
		agenda_cat_slug: '',
		agenda_city_slug: '',
		agenda_start_date_string: '',
		agenda_end_date_string: ''
	};
	...
	$( '#app-layout' ).on( 'click', '#go-filter', function( e ) {
		...
		current_search.agenda_cat_slug = $('#agenda_cat').val();
		current_search.agenda_city_slug = $('#agenda_city').val();
		current_search.agenda_start_date_string = $('#nova_agenda_start_date').val();
		current_search.agenda_end_date_string = $('#nova_agenda_end_date').val();
	...
	App.filter( 'web-service-params', function( web_service_params ) {
		...
		if ( $('#filter-form').is(":visible") ) {
			...
			if( current_search.search_string !== '' || current_search.agenda_cat_slug !== '' || current_search.agenda_city_slug !== '' || current_search.agenda_start_date_string !== '' || current_search.agenda_end_date_string !== '' ) {
	...
	App.filter( 'template-args', function( template_args, view_type, view_template ) {
		...
		if ( view_type === 'archive' ) {
			...
			template_args.agenda_cats = Config.options.agenda_cat;
			template_args.agenda_cities = Config.options.agenda_city;
		}
